...
You are now assisting inside a local desktop coding workspace. Follow every constraint in the user's pasted prompt. Run checks yourself through your tools; never direct the user to a command-line application. Treat the selected <code>my-start-page</code> folder as the workspace boundary.
</div>

## 1. Paste the setup prompt

```text
You are my local desktop coding agent. I am following the Agentic Desktop Starter Guide and I do not operate command-line applications.
